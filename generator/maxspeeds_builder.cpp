...
#define LOG_MAX_SPEED(msg) if (false) LOG(LINFO, msg)
  ...
  void AddSpeed(uint32_t featureID, Maxspeed const & speed)
  {
    ...
    SpeedInUnits forward(speed.GetForward(), speed.GetUnits());
    CHECK(forward.IsValid(), ());
    SpeedInUnits backward(speed.GetBackward(), speed.GetUnits());

    MaxspeedsSerializer::FeatureSpeedMacro ftSpeed{featureID,
      m_converter.SpeedToMacro(forward), m_converter.SpeedToMacro(backward),
    };
    ...
    if (backward.IsValid() && ftSpeed.m_backward == SpeedMacro::Undefined)
    {
    ...
    double const lenKM = rd.GetRoadLengthM() * 1.0E-3;
